...
@Component({
    selector: 'data-views-side-bar',
    templateUrl: './data-views-sideBar.component.html'
})
export class DataViewsSideBarComponent implements OnInit {
    ...
    constructor(
        ...
        //this.jhiLanguageService.setLocations(['fileTemplates']);
    }
    ngOnInit() {
        ...
        this.dataViewsService.sideBarDataViewLists().subscribe((res: any) => {
            ...
            if(this.dataViewsList.length == 0){
                //Do nothing
            }else{
                this.dataViewsList[0].firstOpened = true;
            }
    ...
    getInSidHeight(){
        if(this.dataViewsList.length == 1){
