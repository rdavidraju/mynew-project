...
export class NavbarComponent implements OnInit {
    ...
    themes = [
        ...
        { value: 'red-rose-theme', label: 'Red Rose' },
        { value: 'blue-skies-theme', label: 'Blue Skies' }
    ];
    fonts: SelectItem[];
    ...
    onThemeChange(){
        $('body').removeClass(this.commonService.DYNAMIC_THEME);
        if(this.$localStorage.retrieve('key-theme')){
            this.commonService.DYNAMIC_THEME = this.$localStorage.retrieve('key-theme');
            $('body').addClass(this.commonService.DYNAMIC_THEME);
        } else {
            this.commonService.DYNAMIC_THEME = 'blue-skies-theme';
        }
    }
    ...
    onFontChange(){
        // $('body').removeClass(this.commonService.DYNAMIC_FONT);
        if(this.$localStorage.retrieve('key-font')){
            this.commonService.DYNAMIC_FONT = this.$localStorage.retrieve('key-font');
            // $('body').addClass(this.commonService.DYNAMIC_FONT);
        } else {
    ...
    loadschedularList(){
        //this.getuserinfo();
        this.navbarService.getSchedulersList(1,6).subscribe((resp: Response)=>{
        this.schedularList = [];
    ...
    userSetting(){
        ...
        if(this.currentAccount) {
            let link = ['/user-management', {outlets: {'content': [this.currentAccount.id]+'/details'}}];
