...
export class LedgerDefinitionDetailComponent implements OnInit {
    ...
    saveLedger() {
        if (!this.nameExist)
            this.ledgerDefinitionService.checkLedgerIsExist(this.ledgerDefinition.name, this.ledgerDefinition.id).subscribe(res => {
    ...
    isNameExist(name, id) {
        ...
        this.ledgerDefinitionService.checkLedgerIsExist(name, id).subscribe(res => {
            this.nameExist = res.result != 'No Duplicates Found' ? res.result : undefined;
        });
    }
