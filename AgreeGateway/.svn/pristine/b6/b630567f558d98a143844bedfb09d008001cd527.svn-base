...
export class CalendarDetailComponent implements OnInit{
    ...
    saveCalendar() {
        if (!this.nameExist)
            this.calendarService.checkCalendarNameIsExist(this.calendar.name, this.calendar.id).subscribe(res => {
    ...
    isNameExist(name, id) {
        ...
        this.calendarService.checkCalendarNameIsExist(name, id).subscribe(res => {
            this.nameExist = res.result != 'No Duplicates Found' ? res.result : undefined;
        });
    }
