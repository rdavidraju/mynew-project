...
*/
	@Query(value =  "SELECT * FROM functionality where tenant_id = :tenantId and ((active_ind = 1 AND (DATEDIFF(CURDATE(),start_date)) >= 0) and (DATEDIFF(end_date,CURDATE()) >= 0)) order by func_name asc",nativeQuery=true)
	List<Functionality> fetchByTenantId(@Param("tenantId") Long tenantId);
