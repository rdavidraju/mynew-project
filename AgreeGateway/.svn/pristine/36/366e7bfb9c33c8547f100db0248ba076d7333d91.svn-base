    ...
    */
    ...
    ngOnInit() {
        ...
        this.routeSub = this.route.params.subscribe(params => {
            ...
            if (params['idForDisplay'])
                this.detailId = params['idForDisplay'];
            this.fileTemplate.idForDisplay = params['idForDisplay'];
            if (this.detailId) {
                    ...
                    (res: FileTemplates) => {
                     ...
                     //   console.log('fetched obj is' + JSON.stringify(this.fileTemplate));
                        ...
                        if (this.fileTemplate.multipleRowIdentifiers) {
                            ...
                            this.intermediateTableService.getIntermediateRecords(this.detailId).subscribe((res: any) => {
                                ...
                                this.fileTemplatesLinesService.fetchFTLByTempIdAndInterIDs(this.detailId, interIDs).subscribe((res: any) => {
                                    let resp: any = [];
                                    resp = res;
                                    let temp: number = -1;
                        ...
                        this.fetchSourceProfileAssignments(this.fileTemplate.idForDisplay);

                        if (!this.fileTemplate.multipleRowIdentifiers) {
                            console.log('this.fileTemplate.idForDisplay'+this.fileTemplate.idForDisplay);
                            this.fileTemplatesService.fetchTemplateLinesByTemplateId(this.fileTemplate.idForDisplay).subscribe((res: any) => {
    ...
    saveTemplate() {
        ...
        else if(this.isEdit)
        {
            ...
            this.fileTemplate.startDate=new Date(this.fileTemplate.startDate.getTime() + 86400000);
            if(this.fileTemplate.endDate.getTime())
            this.fileTemplate.endDate=new Date(this.fileTemplate.endDate.getTime() + 86400000);
        }
                ...
                this.sampleDataAfterDropForMI.forEach(MIObj => {
                    ...
                    MIIndex = MIIndex + 1;
                        ...
                        this.notificationsService.error(
                            'No lines found for '+MIObj.rowIdentifier,
                           ''
                        )
                    }
                ...
                if (tempIdAfterSave[0].details) {
                    ...
                    this.fileTemplatesService.fetchById(this.fileTempId).subscribe(
                        (res: FileTemplates) => {
                            ...
                            this.fetchSourceProfileAssignments(this.fileTemplate.idForDisplay);
                            if (!this.createFromRemoteArea) {
                                if (this.fileTemplate.idForDisplay) {
                                    let link: any = '';
                                    link = ['/file-templates', { outlets: { 'content': this.fileTemplate.idForDisplay + '/details' } }];
                                ...
                                if (this.fileTemplate) {
                                    ...
                                    let temp = [];
                                    temp.push(this.fileTemplate.idForDisplay);
    ...
    */
    ...
    fetchSourceProfileAssignments(templateId: any) {
        this.SPAService.fetchSPAByTempId(templateId).subscribe((res: Response) => {
          let sourceProfileAssignments = res;
    ...
    showPositionCols : boolean = false;
    rowIdentifiersClass(){
    ...
    checkForColumnDuplicates(event,i)
    {
        console.log('event'+event);
         if(this.fileTemplate.multipleRowIdentifiers)
         {
            ...
            this.sampleDataAfterDropForMI[this.selectedTabIndex].templateLines.forEach(line=>{
                if(ind != i && line.columnHeader ===  this.sampleDataAfterDropForMI[this.selectedTabIndex].templateLines[i].columnHeader )
                {
                    this.notificationsService.error(
                        
                        line.columnHeader+' already exists',
             ...
             let ind : number =0;
            this.fileTemplateLines.forEach(line=>{
                if(ind != i && line.columnHeader ===   this.fileTemplateLines[i].columnHeader )
                {
                    this.notificationsService.error(
                        line.columnHeader+' already exists',
                        ''
