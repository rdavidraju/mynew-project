...
export class NotificationBatchService {
    ...
    private getViewInfoForEachRefNumOfBatchIdUrl = 'agreeapplication/api/getViewInfoForEachRefNumOfBatchId';
    ...
    fetchListOfNotificationBatch(page: number, per_page: number, moduleType?: any, status?: any,userId?: any): Observable<Response> {
        ...
        }else if (status && status != 'All' && !userId) {
    ...
    getSelectedBatchDetailss(id: any, page: number, per_page: number, status?: any): Observable<Response> {
        console.log('id ' + id + ' page ' + page + ' perpage ' + per_page);
        if (status) {
            return this.http.get(`${this.getViewInfoForEachRefNumOfBatchIdUrl}?notificationId=${id}&status=${status}&pageNumber=${page}&pageSize=${per_page}`).map((res: Response) => {
                return res;
            });
        } else {
            return this.http.get(`${this.getViewInfoForEachRefNumOfBatchIdUrl}?notificationId=${id}&pageNumber=${page}&pageSize=${per_page}`).map((res: Response) => {
                return res;
            });
        }
    }
