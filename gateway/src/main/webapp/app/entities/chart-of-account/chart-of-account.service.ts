...
export class ChartOfAccountService {
    ...
    private getMappingSetValuesBySegLengAndQualifierUrl = 'agreeapplication/api/getMappingSetValuesBySegLengAndQualifier';
    private checkCOAIsExistUrl:any = 'agreeapplication/api/checkCOAIsExist';
    private isChartofAccountUsedUrl = 'agreeapplication/api/checkWhetherCoaTaggedToGroup'
    private activeChartOfAccountsUrl = 'agreeapplication/api/getActivechartOfAccounts';
    private getActiveMappingSetsByTenantIdAndPurposeUrl = 'agreeapplication/api/getActiveMappingSetsByTenantId';
    ...
    updateOrNewsegments(segment, coaId){
        return this.http.put(`${this.deleteOrUpdateOrNewsegmentsUrl}?coaId=${coaId}`, segment).map((res: Response)=>{
            const jsonResponse = res.json();
            return jsonResponse;
        });
    }
    ...
    getChartOfAccountsByTenant(): Observable<any> {
        return this.http.get(this.activeChartOfAccountsUrl).map((res: Response) => {
    ...
    /**
     * @author Sameer
     * @description Get mapping set based on segment length and qualifier
     * @param segLen 
     * @param qualifier 
     */
    getMappingSetValuesBySegLengAndQualifier(segLen, qualifier?) {
        return this.http.get(`${this.getMappingSetValuesBySegLengAndQualifierUrl}?SegmentLength=${segLen}${qualifier ? `&qualifier=${qualifier}` : ''}`).map((res: Response) => {
            return res.json();
        });
    }

    isChartofAccountUsed(id) {
        return this.http.get(`${this.isChartofAccountUsedUrl}?coaId=${id}`).map((res) => res.json());
    }

    getActiveVSByTenantIdAndPurpose(purpose) {
        return this.http.get(`${this.getActiveMappingSetsByTenantIdAndPurposeUrl}?purpose=${purpose}`).map((res) => res.json());
    }
