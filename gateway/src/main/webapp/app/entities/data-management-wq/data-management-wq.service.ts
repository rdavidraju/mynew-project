...
export class DataManagementWqService {
  //private UserData = this.$sessionStorage.retrieve('userData');
    ...
    private findMatchedTemplatesByFileUrl='agreeapplication/api/findMatchedTemplatesBySourceFile';
    ...
    fileTemplatesList  = [];
    private forceTransformUrl = 'agreeapplication/api/force-transform';
    ...
    */
    ...
    search(req?: any): Observable<ResponseWrapper> {
        ...
        return this.http.get(this.searchUrl+'?filterValue='+this.searchData, options)
    ...
    forceTransform(srcFileInbId): Observable<Response> {
        const copy = null;
        return this.http.put(this.forceTransformUrl+'?srcFileInb='+srcFileInbId ,copy).map((res: Response) => {
            return res;
        });
    }
    ...
    fetchSchedulesByProfileOrTemplate(sourceProfileId,templateId?:any) :Observable<Response> {
        /* this.fetchSchedulesByProfileOrTemplateUrl = this.fetchSchedulesByProfileOrTemplateUrl + '?profileId='+sourceProfileId;
        if(templateId)
        {
            this.fetchSchedulesByProfileOrTemplateUrl = this.fetchSchedulesByProfileOrTemplateUrl +'&templateId='+templateId
        } */
        return this.http.get(`${this.fetchSchedulesByProfileOrTemplateUrl}?profileId=${sourceProfileId}${templateId ? `&templateId=${templateId}` : ''}`).map((res: Response) => {
            return res.json();
        });
    }
      ...
      fetchDataMasterLinesBySrcFileInb(page: number, pageSize: number,sourceProfileId,templateId,srcFileHistInbId,interId) :Observable<Response> {
        ...
        return this.http.get(this.fetchDataMasterLinesbySrcFileHistInbUrl + '?sourceProfileId=' + sourceProfileId+'&page='+page+'&per_page='+pageSize+'&templateId='+0+
        '&intermediateId='+intermediateId+'&srcFileHistInbId='+srcFileHistInbId+'&sortColName='+'id'+'&sortOrder='+'desc').map((res: Response) => {
            return res;
        });
    }
      fetchDataStagingLinesBySrcFileInb(page: number, pageSize: number,sourceProfileId,templateId,srcFileHistInbId,linesStatusFilter,interId:any) :Observable<Response> {
        let intermediateId : any;
        if(interId == null || interId === undefined || !interId)
        {
          intermediateId = '';
        ...
        if(linesStatusFilter == 'All')
          ...
          return this.http.get(this.fetchDataStagingLinesbySrcFileHistInbUrl + '?sourceProfileId=' + sourceProfileId+'&page='+page+'&per_page='+pageSize+
                  '&templateId=' +0+ '&intermediateId='+intermediateId+'&srcFileHistInbId='+srcFileHistInbId +'&recordStatus='+linesStatusFilter+'&sortColName='+'id'+'&sortOrder='+'desc').map((res: Response) => {
              return res;
          });
      }
      ...
      {
        return this.http.delete(`${'agreeapplication/api/deleteStagingRecords'}/${srcInbId}`).map((res: Response) => {
            return res.json();
        });
      }
      ...
      fetchProgramList()
      {
          return this.http.get(this.programListByTenantIdUrl ).map((res: Response) => {
              return res.json();
          });
      }
      ...
      accessDrive(fileId)
      {
          return this.http.get('https://www.googleapis.com/drive/v2/files/'+fileId).map((res: Response) => {
              let resp : any = res;
...
//      }
      ...
      public findMatchedTemplatesByFile(files: any): Observable<Response> {
        let formData = new FormData();
        formData.append('file', files);
        let headers = new Headers({});
        let options = new RequestOptions({ headers });
        console.log('in service to fetch matched templates by file');
        return this.http.post(this.findMatchedTemplatesByFileUrl, formData, options).map((res: Response) => {
