...
export class CalendarService {
    ...
    private getCalenderAndItsPeriodsUrl = 'agreeapplication/api/getCalenderAndItsPeriods';
    ...
    private checkCalendarNameIsExistUrl:any = 'agreeapplication/api/checkCalendarNameIsExist';
    ...
    getCalenderAndItsPeriods(id, fileExport?, fileType?){
        if(fileExport){
            return this.http.get(`${this.getCalenderAndItsPeriodsUrl}?id=${id}${fileExport ? `&fileExport=${fileExport}` : ''}${fileExport ? `&fileType=${fileType}` : ''}`,{ responseType: ResponseContentType.Blob }).map((res: Response)=>{
                return fileExport ? res : res.json();
            });
        }else{
            return this.http.get(`${this.getCalenderAndItsPeriodsUrl}?id=${id}${fileExport ? `&fileExport=${fileExport}` : ''}${fileExport ? `&fileType=${fileType}` : ''}`).map((res: Response)=>{
                return fileExport ? res : res.json();
            });
        }
    }
