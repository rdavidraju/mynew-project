...
export class RulesService {
   ...
   // private UserData = this.$sessionStorage.retrieve('userData');
    ...
    getRules(rulePurpose: any,ruleType: any,appProcess?:any){
        if(!ruleType) {
            ruleType ='';
            }
        ...
        return this.http.get(this.fetchRulesByRulePurpose+'?rulePurpose='+rulePurpose+'&ruleType='+ruleType+'&appProcess='+appProcess).map((res: Response) => {
    ...
    getAccountingRules(rulePurpose: any):Observable<any>{
        ...
        return this.http.get(this.fetchAccountinrules+'?rulePurpose='+rulePurpose).map((res: Response) => {
            const jsonResponse = res.json();
            return jsonResponse;
        });
    }
     ...
     * @param rulePurpose
     */
    getAccountingRulesList(rulePurpose : any,coa?:any,activityBased?:boolean):Observable<any>{
        console.log(' in service of get accounting rule list');
        return this.http.get(this.accountingRuleListUrl+'?rulePurpose='+rulePurpose+'&coaId='+coa+'&activityBased='+activityBased).map((res: Response) => {
            const jsonResponse = res.json();
            return jsonResponse;
        });
    }
    getApprovalRules(ruleId : any):Observable<any>{
        
        return this.http.get(this.fetchApprovalrules+'?ruleId='+ruleId).map((res: Response) => {
            let jsonResponse = res.json();
            return jsonResponse;
        });
    }
