...
import { SourceProfiles, JustJson } from './source-profiles.model';
import { ResponseWrapper, createRequestOption } from '../../shared';
@Injectable()
export class SourceProfilesService {

    public sourceProfileJSON =
    {
        "com.nspl.app.domain.SourceProfiles": [
            {
                "Source Profile Name":"sourceProfileName",
    ...
    public sourceProfileFileAssignmentsJSON =
    {
        "com.nspl.app.domain.SourceProfileFileAssignments": [
            {
                "File Template":"fileNameFormat",
            ...
            {
                "parentTable": "com.nspl.app.domain.SourceProfiles",
                "parentColumn": "sourceProfileName"
            }
        ]
    };
    public sourceConnectionDetailsJSON =
    {
        "com.nspl.app.domain.SourceConnectionDetails": [
   ...
   // private UserData = this.$sessionStorage.retrieve('userData');
    ...
    private saveProfileAndTagTemplateUrl ='agreeapplication/api/sourceProfileAndTemplate';
    public searchData:any;
    private searchBatchData:any;
    ...
    search(req?: any,sortColName?:any,sortOrder?:any): Observable<ResponseWrapper> {
        let options = createRequestOption(req);
        return this.http.get(this.resourceSearchUrl+'?filterValue='+this.searchData+'&sortColName='+sortColName+'&sortOrder='+sortOrder, options)
    ...
    getProfilesAndConnectionsByProfileId( Id: any ): Observable<Response> {
       ...
       // console.log('id' + Id);
        return this.http.get( `${this.sourceProfilesWithConnectionDetails}/?idForDisplay=${Id}` ).map(( res: Response ) => {
            let jsonResponse = res.json();
           // console.log('jsonResponse;;;;'+JSON.stringify(jsonResponse));
            jsonResponse = jsonResponse[0];
            console.log('jsonResponse.startDate before conversion'+jsonResponse.startDate);
            jsonResponse.startDate = this.dateUtils
               .convertDateTimeFromServer(jsonResponse.startDate);
               console.log('jsonResponse.startDate after conversion'+jsonResponse.startDate);
     ...
     */
    getAllProfilesAndConnections(status?): Observable<Response> {
        return this.http.get( `${this.sourceProfilesWithConnectionDetails}${status ? `?isActive=${status}` : ''}` ).map(( res: Response ) => res );
     }
    ...
    {  
    // console.log('before save profile has'+JSON.stringify(srcProfilesAndAssignments));
       ...
       console.log('start date in service'+srcProfilesAndAssignments.sourceProfiles.startDate);
    ...
    postProfileAndTagTemplate(sourceProfilesAndTagTemplate): Observable<SourceProfiles> {
        
        return this.http.post(this.saveProfileAndTagTemplateUrl, sourceProfilesAndTagTemplate).map((res: Response) => {
            const jsonResponse = res.json();
            this.convertItemFromServer(jsonResponse);
            return jsonResponse;
        });
