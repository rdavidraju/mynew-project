...
export class TemplateDetailsDetailComponent implements OnInit, OnDestroy {
    ...
    fetchJournalsDetails() {
            ...
            if (params['id']) {
                this.templateDetailsService.getJournalsById(params['id']).subscribe(
                    (journalsHeaderDataService) => {
                        ...
                        this.journalsHeader.startDate = this.dateUtils.convertDateTimeFromServer(this.journalsHeader.startDate);
                        this.journalsHeader.endDate = this.dateUtils.convertDateTimeFromServer(this.journalsHeader.endDate);
                        ...
                        if(this.journalsHeader.jeLineDerivations){
                            const tempAry = [];
                            for (let a = 0; a < this.journalsHeader.jeLineDerivations.length; a++) {
                                tempAry.push(this.journalsHeader.jeLineDerivations[a].sViews);
                            }
                        ...
                        for (let i = 0; i < this.journalsHeader.jeHeaderDerivations.length; i++) {
        ...
        this.templateDetailsService.getLookUps('JE_DERIVATIONS').subscribe((res: any) => {
            ...
            this.templateDetailsService.getLookUps('MAPPING_TYPE').subscribe((res1: any) => {
                this.mappingTypes = res1;
                console.log('testMapping'+JSON.stringify(this.mappingTypes));
                for (let i = 0; i < this.journalHeaders.length; i++) {
    ...
    convertMappingSetPurpose() {
        ...
        const tempArray = [];
        const tempArray2 = [];
        for (let i = 0; i < this.journalsHeader.selectedView.length; i++) {
    ...
    saveJournalData() {
        ...
        for (let i = 0; i < this.jeLineDerivations.length; i++) {
    ...
    testChange(obj: any, ind: any) {
        ...
        const indVal = '#disp_'.concat(ind + 1);
    ...
    onViewSelect(obj: any) {
        ...
        for (let a = 0; a < this.journalHeaders.length; a++) {

            this.journalHeaders[a].viewId = '';
            this.journalHeaders[a].mappingType = '';
            this.journalHeaders[a].value = '';
        }
        ...
        for (let j = 0; j < this.selectedViews.length; j++) {
            console.log('obj[j].id ' + this.selectedViews[j].id);
            this.dataViewsService.getDVById(this.selectedViews[j].id).subscribe((res: any) => {
                ...
                if (this.selectedViewArr[0].dataViewsColumnsList != undefined) {
                    for (let a = 0; a < this.selectedViewArr[0].dataViewsColumnsList.length; a++) {
                        this.dataViewCols.push(this.selectedViewArr[0].dataViewsColumnsList[a]);
                    ...
                    for (let x = 0; x < this.selectedViewArr[0].dataViewsUnionColumnsList.length; x++) {
        ...
        if (this.presentUrl.endsWith('edit')) {
            ...
            this.jeLineDerivations.push(obja);
        }
        $('#disp_1').css('display', 'none');
        $('#disp_2').css('display', 'none');
        $('#disp_3').css('display', 'none');
    ...
    getColumns(val: any, ind) {
        ...
        for (let i = 0; i < this.dataViewCols.length; i++) {
    ...
    mapval(val: any, ind) {
        ...
        for (let i = 0; i < this.mappingTypes.length; i++) {
