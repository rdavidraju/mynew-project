...
public class DashBoardV3Service {
	...
	public List<LinkedHashMap> datesService(LocalDate startDate,LocalDate endDate)
	{
		...
		for(int i=0;i<datesList.size();i=i+2)
		{
			...
			{
				edDate= datesList.get(i).toInstant()
					.atZone(ZoneId.systemDefault())
					.toLocalDate();
			}
			else
			{
				edDate= datesList.get(i+1).toInstant()
						.atZone(ZoneId.systemDefault())
						.toLocalDate();
			}
	...
	public List<LinkedHashMap> weeksOfAMonth(LocalDate originalStartDate,LocalDate originalEndDate)
	{
		List<LinkedHashMap> dateRangeList = new ArrayList<LinkedHashMap>();

		LocalDate weekStartDate=originalStartDate;
		LocalDate weekEndDate=originalStartDate.plusDays(6);

		LinkedHashMap map = new LinkedHashMap();
		if(weekEndDate.isBefore(originalEndDate))
		{
			map.put("startDate", weekStartDate);
			map.put("endDate", weekEndDate);
		}
		else
		{
			map.put("startDate", weekStartDate);
			map.put("endDate", originalEndDate);
		}
		dateRangeList.add(map);

		while (weekEndDate.isBefore(originalEndDate))
		{
			weekStartDate = weekEndDate.plusDays(1);
			weekEndDate = weekStartDate.plusDays(6);

			LinkedHashMap week = new LinkedHashMap();
			if(weekEndDate.isBefore(originalEndDate))
			{
				week.put("startDate", weekStartDate);
				week.put("endDate", weekEndDate);
			}
			else
			{
				week.put("startDate", weekStartDate);
				week.put("endDate", originalEndDate);
			}
			dateRangeList.add(week);
