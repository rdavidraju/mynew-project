...
public class TenantConfigModulesResource {
    ...
    @Timed
    public ResponseEntity<TenantConfigModules> createTenantConfigModules(@RequestBody TenantConfigModules tenantConfigModules, @RequestParam String tenantId) throws URISyntaxException {
    	log.debug("REST request to save TenantConfigModules : {}", tenantConfigModules+" and tenantId: "+tenantId);
    	//String tenantId=tenantConfigModules.getTenantId().toString();
    	HashMap tenatId=new HashMap();
    	try {
    		tenatId=userJdbcService.getTenantIdForDisplay(tenantId);
    	...
    	if(tenatId!=null){//log.info("tenantId: "+tenantId);
    	tenantConfigModules.setTenantId(Long.valueOf(tenatId.get("tenantId").toString()));}
    	else
...
//        if (tenantConfigModules.getId() == null) {
//            return createTenantConfigModules(tenantConfigModules, tenantConfigModules.getTenantId().toString());
//        }
        tenantConfigModules.setStartDate(tenantConfigModules.getStartDate());
        tenantConfigModules.setEndDate(tenantConfigModules.getEndDate());
        TenantConfigModules result = tenantConfigModulesRepository.save(tenantConfigModules);
    ...
    public List<TenantConfigModulesDTO> getTenantConfigModulesByTenantId(HttpServletRequest request,@RequestParam(value="tenantId",required=false) String tenantId) throws ClassNotFoundException, SQLException {
    	...
    	List<TenantConfigModules> tenantConfigModulesList = tenantConfigModulesRepository.findByTenantId(tenId);
    	...
    	for(int i=0;i<tenantConfigModulesList.size();i++){
    		...
    		/** active check**/
			Boolean activeStatus=activeStatusService.activeStatus(tenantConfigMod.getStartDate(), tenantConfigMod.getEndDate(), tenantConfigMod.isEnabledFlag());
    		tenantConfigModData.setEnabledFlag(activeStatus);
