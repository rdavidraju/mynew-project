...
public interface DataViewsSrcMappingsRepository extends JpaRepository<DataViewsSrcMappings,Long> {
	...
	DataViewsSrcMappings findByDataViewIdAndTemplateIdAndIntermediateId(Long dataViewId, Long templateId, Long intermediateId);
	
	DataViewsSrcMappings findByDataViewIdAndTemplateId(Long dataViewId, Long templateId);
	
	@Query(value="SELECT template_id,intermediate_id FROM t_data_views_src_mappings where data_view_id = :dataViewId group by template_id,intermediate_id",nativeQuery=true)
	List<Object[]> fetchDVUniqueCombinations(@Param(value = "dataViewId") Long dataViewId);
	
	@Query(value="select distinct relation from t_data_views_src_mappings where data_view_id= :dataViewId and relation is not null",nativeQuery=true)
	String fetchDVRelationByViewId(@Param(value = "dataViewId") Long dataViewId);
