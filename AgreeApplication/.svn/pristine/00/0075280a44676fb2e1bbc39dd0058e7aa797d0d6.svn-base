...
//	Page<SchedulerDetails> fetchSchedulersByJobId(Long tenantId,Pageable generatePageRequest2);
	...
	@Query(value =  "SELECT * FROM t_scheduler_details where job_id in (:job) and is_viewed is false",nativeQuery=true)
	List<SchedulerDetails> findByJobIdInAndIsViewedFalse(@Param("job") List<BigInteger> job);
