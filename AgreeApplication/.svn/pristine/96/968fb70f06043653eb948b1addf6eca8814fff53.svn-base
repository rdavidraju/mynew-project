...
public interface AccountedSummaryRepository extends JpaRepository<AccountedSummary,Long> {
	...
	@Query(value =  "SELECT row_id FROM t_accounted_summary where rule_group_id = :ruleGroupId and view_id = :viewId and status='ACCOUNTED' and current_record_flag is true",nativeQuery=true)
...
//	List<BigInteger> fetchAllAccountedRowIdsByRuleGrpIdAndViewId( @Param("ruleGroupId") Long ruleGroupId, @Param("viewId") Long viewId);
	...
	//journal status filter
		...
		@Query(value = "SELECT row_id FROM t_accounted_summary where rule_group_id = :ruleGroupId and view_id = :viewId and status = :status and current_record_flag is true and (journal_status='' or journal_status is null)",nativeQuery=true)
		List<BigInteger> fetchJEPendingIdsByStatusNGroupIdNViewId(@Param("ruleGroupId") Long ruleGroupId, @Param("viewId") Long viewId, @Param("status") String status);
		...
		@Query(value =  "SELECT row_id FROM t_accounted_summary where rule_group_id = :ruleGroupId and view_id = :viewId and status ='ACCOUNTED' and (journal_status is not null and journal_status='ENTERED') AND current_record_flag is true",nativeQuery=true)
		...
		@Query(value =  "SELECT count(*) FROM t_accounted_summary where rule_group_id = :ruleGroupId and view_id = :viewId and rule_id = :ruleId and  status='ACCOUNTED' and current_record_flag is true",nativeQuery=true)
		...
		@Query(value =  "SELECT * FROM t_accounted_summary where id in (:ids)",nativeQuery=true)
		List<AccountedSummary> findByIdIn(@Param("ids") List<BigInteger> ids);

		List<AccountedSummary> findByRuleGroupId(Long id);
		
		@Query(value =  "select row_id from t_accounted_summary where rule_group_id = :ruleGroupId and view_id = :viewId and (journal_status is null AND (approval_status is null OR approval_status = 'REJECTED')) AND row_id in (:rowIds)",nativeQuery=true)
		List<BigInteger> fetchEligibleRowIdsForUnAccounting(@Param("ruleGroupId") Long ruleGroupId, @Param("viewId") Long viewId, @Param("rowIds") List<BigInteger> rowIds);
