...
public interface SourceProfilesRepository extends JpaRepository<SourceProfiles,Long> {
	...
	// kiran
	Page<SourceProfiles> findByTenantId(Long tenantId,Pageable pageable);
	
	@Query(value =  "SELECT id FROM t_source_profiles where connection_id in (SELECT id FROM t_source_connection_details WHERE tenant_id=:tenantId order by name)",nativeQuery=true)
	List<Long> fetchSourcePrfliesIdsUsingConnections(@Param("tenantId") Long tenantId);
	
//	@Query(value =  "SELECT id FROM t_source_profiles where id in (:tenantIds)",nativeQuery=true)
//	Page<SourceProfiles> fetchSourcePrfliesAndConnections(@Param("tenantId") List<Long> tenantIds, Pageable pageable);
	
	Page<SourceProfiles> findByIdIn(List<Long> profId,Pageable pageable);

	SourceProfiles findByTenantIdAndIdForDisplay(Long tenantId,String idForDisplay);

	

	List<SourceProfiles> findByTenantIdAndEnabledFlagIsTrueOrderByIdDesc(
			Long tenantId);
