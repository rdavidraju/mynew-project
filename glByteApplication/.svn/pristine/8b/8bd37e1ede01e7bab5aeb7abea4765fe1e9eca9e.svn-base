...
public interface RulesRepository extends JpaRepository<Rules,Long> {
	...
	@Query(value =  "SELECT * FROM t_rules where id in (:ids) and tenant_id= :tenantId and coa= :coaId and enabled_flag is true",nativeQuery=true)
	List<Rules> findByIdsAndTenantIdAndCoaAndEnabledFlag(@Param("ids") List<Long> ids,@Param("tenantId") Long tenantId,@Param("coaId") Long coaId);
	...
	@Query(value =  "SELECT distinct(source_data_view_id) FROM t_rules where id in (:ids) and source_data_view_id is not null",nativeQuery=true)
	List<BigInteger>  fetchDistictSrcViewIdsByRuleId(@Param("ids") List<Long> ids);
	
	@Query(value =  "SELECT distinct(target_data_view_id) FROM t_rules where id in (:ids) and target_data_view_id is not null",nativeQuery=true)
	...
	@Query(value =  "SELECT distinct(target_data_view_id) FROM t_rules where source_data_view_id= :source_data_view_id and tenant_id=:tenantId and id in(:ruleIds) and target_data_view_id is not null",nativeQuery=true)
	...
	// Kiran
	@Query(value = "SELECT distinct(source_data_view_id) FROM t_rules where id in (SELECT rule_id FROM t_rule_group_details where rule_group_id =:ruleGroupId) "
			+ "and target_data_view_id=:viewId UNION ALL SELECT distinct(target_data_view_id) FROM t_rules where id in "
			+ "(SELECT rule_id FROM t_rule_group_details where rule_group_id =:ruleGroupId) and source_data_view_id=:viewId",nativeQuery=true)
	List<Long> fetchDataViewIdsByRuleGrpAndViewIds(@Param("ruleGroupId") Long ruleGroupId, @Param("viewId") Long viewId);
	
	
	@Query(value = "SELECT distinct(source_data_view_id) FROM t_rules where id in (SELECT rule_id FROM t_rule_group_details where rule_group_id =:ruleGroupId) "
			+ "UNION ALL SELECT distinct(target_data_view_id) FROM t_rules where id in (SELECT rule_id FROM t_rule_group_details where rule_group_id =:ruleGroupId)",nativeQuery=true)
	List<Long> fetchDataViewIdsByRuleGrpIds(@Param("ruleGroupId") Long ruleGroupId);

	@Query(value = "SELECT id,rule_code FROM t_rules where source_data_view_id= :srcDataViewId and target_data_view_id= :tgtDataViewId and id in (select rule_id from t_rule_group_details where rule_group_id in (select id from t_rule_group where id= :ruleGroupId))",nativeQuery=true)
	List<Object[]> fetchRulesForSrcNTrgtViews(@Param("srcDataViewId") Long srcDataViewId,@Param("tgtDataViewId") Long tgtDataViewId,@Param("ruleGroupId") Long ruleGroupId);
	
	
	@Query(value = "SELECT id FROM t_rules where source_data_view_id= :srcDataViewId and target_data_view_id is null and id in (select rule_id from t_rule_group_details where rule_group_id in (select id from t_rule_group where id= :ruleGroupId))",nativeQuery=true)
	List<BigInteger> fetchRulesForSrcNViewsForAccounting(@Param("srcDataViewId") Long srcDataViewId,@Param("ruleGroupId") Long ruleGroupId);



	@Query(value = "select * from t_rule_group where id = (select rule_group_id from t_rule_group_details where rule_id=(SELECT id FROM t_rules where rule_code=:ruleName)) and rule_purpose=:rulePurpose and tenant_id=:tenantId",nativeQuery=true)
	Object findRuleWithNameAndTenantId(@Param("ruleName") String ruleName, @Param("tenantId") Long tenantId, @Param("rulePurpose") String rulePurpose);
	
	
	@Query(value = " select * from t_rules where source_data_view_id in (SELECT distinct(source_data_view_id) FROM t_rules where id in"
			+ " (SELECT rule_id FROM t_rule_group_details where rule_group_id =:ruleGroupId) "
			+ " UNION ALL "
			+ "(SELECT distinct(target_data_view_id) FROM t_rules where id in "
			+ "(SELECT rule_id FROM t_rule_group_details where rule_group_id =:ruleGroupId))) and id in(:ids)",nativeQuery=true)
	List<Rules> fetchFilteredRulesByReconOrAccRuleGroup(@Param ("ids") List<Long> ids,@Param("ruleGroupId") Long ruleGroupId);

	
	@Query(value =  "select id from t_rules where id in (:ids) and source_data_view_id = :srcViewId",nativeQuery=true)
	List<Long> fetchIdsBySourceViewIdAndRuleIds(@Param ("ids") List<Long> ids, @Param("srcViewId") Long srcViewId);

	
	@Query(value =  "select distinct currency_column_id from t_rules where id in (:ids)",nativeQuery=true)
	List<BigInteger> fetchIdsCurrencyColIdsByRuleIds(@Param ("ids") List<Long> ids);


	List<Rules> findByTenantIdAndCOA(Long tenantId, String id);
