...
public interface ReportDefinationRepository extends JpaRepository<ReportDefination,Long> {
	...
	@Query(value = "select * from t_report_defination where report_id=:reportId and data_type IN ('DATE','VARCHAR') AND layout_val is not null ORDER BY CAST(layout_val AS DECIMAL)",nativeQuery=true)
	List<ReportDefination> getColNames(@Param("reportId") Long reportId);
	
	@Query(value = "select * from t_report_defination where report_id=:reportId and data_type = 'DECIMAL' AND layout_val is not null ORDER BY CAST(layout_val AS DECIMAL)",nativeQuery=true)
	List<ReportDefination> getDecimalColNames(@Param("reportId") Long reportId);
